...
// ФИКС относительно прошлой версии: `Typography()` без параметров — это
// пустой объект-заглушка, Compose всё равно подставляет дефолтную Material
// типографику (Roboto, стандартные размеры) — она и так работала бы, но
// неявно и без возможности тонкой настройки. Явная шкала даёт контроль
// над жирностью/интерлиньяжем в ключевых местах (например, крупная цифра
// стрика на Dashboard использует displayMedium с увеличенным весом).
